 ...
 * @return 0: 성공, 음수: 실패
 */
int DMSDCM_ProcessRedisPUBLISHToKey(DMSDCM_MIB *mib, KVHREDISKey *key, uint8_t *data, size_t data_size) 
{
	int ret;
 ...
 * @return 0: 성공, 음수: 실패
 */
int DMSDCM_ProcessRedisSETtoKey(DMSDCM_MIB *mib, KVHREDISKey *key, uint8_t *data, size_t data_size) 
{
	int ret;
 ...
 * @param[in] mib 모듈 통합관리정보 포인터
 */
int DMSDCM_SetRedisKey(KVHREDISKey *key, KVHREDISKeySUBDOMAIN subdomain, KVHREDISKeyDataModelType datamodel)
{
	if (key == NULL) {
		return -1;
	}
	...
	switch (subdomain) {
		case kKVHREDISKeySUBDOMAIN_IVDCT:
			KVHREDIS_SetKeyStaticField(key, subdomain, "MDSM", 7, datamodel, kKVHREDISKeyDataModelSubType_None);
			break;
		default:
			return -3;
	}
 ...
 * @param[in] mib 모듈 통합관리정보 포인터
 */
int DMSDCM_InitRedisIF(DMSDCM_MIB *mib)
{
   ...
   */
	...
	ret = DMSDCM_SetRedisKey(&mib->redis_if.key_ivdct_protobuf, kKVHREDISKeySUBDOMAIN_DMS, kKVHREDISKeyDataModelType_Protobuf);
	if (ret != 0) {
		ERR("Fail to add Redis Protobuf Key\n");
		return -1;
	}
	ret = DMSDCM_SetRedisKey(&mib->redis_if.key_ivdct_raw, kKVHREDISKeySUBDOMAIN_DMS, kKVHREDISKeyDataModelType_Raw);
	if (ret != 0) {
		ERR("Fail to add Redis Raw Key\n");
		return -1;
	}

	sprintf(mib->redis_if.key_ivdct_publish.key_str, "%s", DMSDCM_REDIS_PUBLISH_KEY);
