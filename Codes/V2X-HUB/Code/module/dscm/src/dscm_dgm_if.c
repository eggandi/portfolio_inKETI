 ...
 *   - C-ITS 서비스 데이터
 */
static void DSCM_ProcessRxUDSDataFromDGM(KVHUDSHANDLE h, uint8_t *data, KVHIFDataLen len, void *priv)
{
  DSCM_MIB *mib = (DSCM_MIB *)priv;
  mib->trx_cnt.dgm.rx++;
  ...
  if (uds_data->hdr.dst != kKVHUDSEntityType_IVN_ADEM) {
 ...
 * @retval -1: 실패
 */
int DSCM_TransmitUDSDataToDGM(DSCM_MIB *mib, uint8_t *data, KVHIFDataLen len)
{
  mib->trx_cnt.dgm.tx++;
  ...
  if (KVHTCP_SendToServer(mib->dgm_if.uds.h, data, len) < 0) {
 ...
 * @retval -1: 실패
 */
int DSCM_InitDGMIF(DSCM_MIB *mib)
{
